...
OPPONENT_CORNER_TOP = {
    "x": -1700.0,
    "y": 1000.0,
}  # Canto superior adversário (Ajuste o Y)
OPPONENT_CORNER_BOTTOM = {
    "x": -1700.0,
    "y": -1000.0,
}  # Canto inferior adversário (Ajuste o Y)
...
class Strategy:
    ...
    def move_to_target(self, robot_info, target_pos, components, kick=False):
        ...
        robot_target_orientation = target_pos.get(
            "orientation", 0.0
        )  # Padrão é 0 se não fornecido
        ...
        if sender:
            sender.send_command(
                wheel_speeds["fl_speed"],
                wheel_speeds["fl_direction"],
                wheel_speeds["fr_speed"],
                wheel_speeds["fr_direction"],
                wheel_speeds["bl_speed"],
                wheel_speeds["bl_direction"],
                wheel_speeds["br_speed"],
                wheel_speeds["br_direction"],
                kick,
            )
    ...
    def decide_action(self, robot_state, robot_id):
        ...
        target_orientation = math.atan2(
            (robot_target_y - robot_current_y), (robot_target_x - robot_current_x)
        )

        distance_to_ball = math.sqrt(
            (robot_target_x - robot_current_x) ** 2
            + (robot_target_y - robot_current_y) ** 2
        )
    ...
    def play_game(self, detection, components):
        ...
        if detection:
            ...
            )  # if TEAM_COLOR == "blue" else detection.robots_yellow
            # opponent_robots = (
            ...
            if gc_data is not None:
                ...
                print(
                    f"[GC] Command: {game_state}, Next: {next_command}, Stage: {stage}, Time left: {time_left}"
                )
            else:
            ...
            for robot_info in our_robots:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error processing robot {robot_id}: {e}", exc_info=True
                    )
    ...
    def attack(self, robot_info, ball_info, components):
        ...
        target_orientation = math.atan2(
            vec_ball_to_target["y"], vec_ball_to_target["x"]
        )
        ...
        dist_to_target = math.hypot(
            target_pos["x"] - robot_info.x, target_pos["y"] - robot_info.y
        )
        ...
        if is_in_position:
            ...
            print(
                f"Atacante: Movendo (Dist: {dist_to_target:.0f}mm, AngErr: {math.degrees(orientation_error):.0f}deg)"
            )
    ...
    def defender(self, robot_info, ball_info, components):
        ...
        dist_to_ball = math.hypot(
            ball_info.x - robot_info.x, ball_info.y - robot_info.y
        )
        ...
        if dist_to_ball < GOALIE_CLEARANCE_DISTANCE:
            ...
            target_orientation = math.atan2(
                (ball_info.y - robot_info.y), (ball_info.x - robot_info.x)
            )
            target_pos = {
                "x": target_x,
                "y": target_y,
