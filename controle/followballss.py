...
This class present the PID controller implementation on the mobile robot follower ball, this class present five
functions the first function called init, make the initialization of the variables, the connect_CRB present
the connection withthe coppeliaSim, Speed_CRB calculated the wheels speeds based on the robot topology
(in this case is a differential robot),PID_Controller_phi implement the PID based on the phi error (see video
of the georgia tech course in this link:
...
Finally comment:
    Querido lector por ahora, la correcta implementación de este código lo sabe Mario, Dios, la Virgen Maria y los
    santos esperemos que futuramente Mario continue sabiendo como ejecutar el código o inclusive que más personas
    se unan para que el conocimiento aqui depositado se pueda expandir de la mejor manera.
    Let's go started =)
""" ""
...
class Corobeu:
    ...
    Corobeu class is the principal class to controller the EVA positions robots.
        ...
        Speed_CRB               = Calculated the wheels speeds based on the robot topology
                                (in this case is a differential robot)
    ...
    def connect_CRB(self, port):
        ...
        returnCode, MotorE1 = sim.simxGetObjectHandle(
            clientID, "Me1", sim.simx_opmode_blocking
        )
        returnCode, MotorE2 = sim.simxGetObjectHandle(
            clientID, "Me2", sim.simx_opmode_blocking
        )
        returnCode, MotorD1 = sim.simxGetObjectHandle(
            clientID, "Md1", sim.simx_opmode_blocking
        )
        returnCode, MotorD2 = sim.simxGetObjectHandle(
            clientID, "Md2", sim.simx_opmode_blocking
        )

        returnCode, ball = sim.simxGetObjectHandle(
    ...
    def PID_Controller_phi(
        self, kp, ki, kd, deltaT, error, interror, fant, Integral_part
    ):
        """""
        ...
        if sim.simxGetConnectionId(clientID) != -1:
            ...
            while a == 1:
                ...
                if Number_Iterations <= 1:
                    ...
                    s, positiona = sim.simxGetObjectPosition(
                        clientID, robot, -1, sim.simx_opmode_streaming
                    )
                    s, ballPos = sim.simxGetObjectPosition(
                        clientID, ball, -1, sim.simx_opmode_streaming
                    )
                    s, angle_robot = sim.simxGetObjectOrientation(
                        clientID, robot, -1, sim.simx_opmode_blocking
                    )
                    # self.phi = angle_robot[2] - 1.47
                    self.phi = 0
                    sim.simxSetJointTargetVelocity(
                        clientID, E1, 0, sim.simx_opmode_blocking
                    )
                    sim.simxSetJointTargetVelocity(
                        clientID, E2, 0, sim.simx_opmode_blocking
                    )
                    sim.simxSetJointTargetVelocity(
                        clientID, D1, 0, sim.simx_opmode_blocking
                    )
                    sim.simxSetJointTargetVelocity(
                        clientID, D2, 0, sim.simx_opmode_blocking
                    )
                else:
                    ...
                    phid = math.atan2(
                        ballPos[1] - positiona[1], ballPos[0] - positiona[0]
                    )
                    ...
                    print(
                        f"error degree == > {error_phi_degree}, error ==> {error_phi}"
                    )
                    omega, fant_phi, interror_phi, Integral_part_phi = (
                        self.PID_Controller_phi(
                            kpi,
                            kii,
                            kdi,
                            deltaT,
                    ...
                    error_distance = math.sqrt(
                        (ballPos[1] - positiona[1]) ** 2
                        + (ballPos[0] - positiona[0]) ** 2
                    )
                    ...
                    vl, vd, a = self.Speed_CRB(
                        self.v_linear, omega, error_distance, Number_Iterations
                    )

                    # Send the speed values to coppeliasim simulato #

                    sim.simxSetJointTargetVelocity(
                        clientID, E1, -vl, sim.simx_opmode_blocking
                    )
                    sim.simxSetJointTargetVelocity(
                        clientID, E2, -vl, sim.simx_opmode_blocking
                    )
                    sim.simxSetJointTargetVelocity(
                        clientID, D1, vd, sim.simx_opmode_blocking
                    )
                    sim.simxSetJointTargetVelocity(
                        clientID, D2, vd, sim.simx_opmode_blocking
                    )
