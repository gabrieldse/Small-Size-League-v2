...
const uint8_t PACKET_LEN =
    14; //  Header 1 e 2 + ID(1)+9 dados+checksum(1)+tail(1)
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    // se o buffer circular estiver cheio, avança tail para não sobrescrever
    // dados
    if (head == tail_idx) {
  ...
  while (buffer_len() >= PACKET_LEN) {
    ...
    if (buffer[idx0] == HEADER[0] && buffer[idx1] == HEADER[1]) {
      ...
      if (buffer[tail_packet_idx] == TAIL) {
        ...
        uint8_t checksum_calculado = calc_sum(
            &final_packet[data_start_idx], 9); // Calcula sobre 9 bytes de dados
